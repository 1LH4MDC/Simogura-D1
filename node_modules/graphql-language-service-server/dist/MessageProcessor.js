...
class MessageProcessor {
    constructor({ logger, fileExtensions, graphqlFileExtensions, loadConfigOptions, config, parser, tmpDir, connection, }) {
        ...
        (0, node_fs_1.mkdirSync)(this._tmpDirBase, { recursive: true });
    }
    get connection() {
    ...
    _getCustomLocateResult(project, result, locateCommand) {
        ...
        try {
            ...
            if (typeof locateResult === 'string') {
                ...
                return {
                    uri,
                    range: new graphql_language_service_1.Range(new graphql_language_service_1.Position(startLine, startCharacter), new graphql_language_service_1.Position(startLine, startCharacter)),
                };
            }
            return locateResult;
        }
    ...
    async _cacheSchemaFile(fileUri, project) {
        try {
            const matches = await (0, fast_glob_1.glob)(fileUri, {
                cwd: project.dirpath,
                absolute: true,
            });
            const uri = matches[0];
